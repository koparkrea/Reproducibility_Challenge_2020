...
# criteria forward
def criteria_forward(inputs, conv, index_list, criteria):
...
class ConvBNRelu(nn.Module):
    ...
    def search_forward(self, tuple_inputs):
        ...
        out_criteria_list = []
        for nu, set_ in enumerate(criteria_list):
            out_align_list = [] 
            ratio_index = [self.choices[wid] for wid in width]
            out_conv, output_channel_index_list = criteria_forward(out, self.conv, ratio_index, set_)
            for i in range(len(width)):
                out_align = self.BNs(Align(out_conv[i], output_channel_index_list[i], self.out_dim)) * prob[i]
                out_align_list.append(out_align)
            out_prob = sum(out_align_list)
            out_criteria = out_prob * criteria_set_prob[nu]
            out_criteria_list.append(out_criteria)
        out = sum(out_criteria_list)
        #out = self.BNs(out)
...
class SearchWidthCifarResNet(nn.Module):
    ...
    def search_forward(self, inputs):
        ...
        selected_widths, selected_probs = selectwithP(self.width_attention, self.tau, num = 5)
