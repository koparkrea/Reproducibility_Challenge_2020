...
from procedures.utils_criteria import get_width_choice, addictive_func, Align
...
def BN_forward(output, BN, output_channel_index):
    out_bn = BN(output)
    out_bn = out_bn[:,output_channel_index]
    return out_bn
...
class ConvBNRelu(nn.Module):
    ...
    def __init__(self, nin, nout, kernel, stride, padding, bias, has_avg, has_bn, has_relu):
        ...
        self.has_bn = has_bn
        self.BNs = nn.BatchNorm2d(self.choices[-1])
    ...
    def search_forward(self, tuple_inputs):
        ...
        for i in range(len(width)):
            ...
            out_align_list = []
            for nu in range(len(criteria_list)):
                #out_align = Align(out_conv[nu], output_channel_index_list[nu], self.out_dim) * criteria_set_prob[nu]
                out_bn = BN_forward(out_conv, self.BNs, output_channel_index_list[nu]) * criteria_set_prob[nu]
                out_align = Align(out_bn, output_channel_index_list[nu], self.out_dim)
                out_align_list.append(out_align)
            out_criteria = sum(out_align_list)
            out_criteria_list.append(out_criteria)
        out = out_criteria_list[0] * prob[0] + out_criteria_list[1] * prob[1]
        #out = self.BNs(out)
...
class ResNetBasicblock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride):
        ...
        self.conv_a = ConvBNRelu(inplanes, planes, 3, stride, 1, False, False, True, True)
        self.conv_b = ConvBNRelu(planes, planes, 3, 1 ,1, False, False, True, False)
    ...
    def search_forward(self, tuple_inputs):
        ...
        assert len(widths) == 2 and prob.size(0) == 2
...
class SearchWidthCifarResNet(nn.Module):
    def __init__(self, block_name, depth, num_classes):
        ...
        nn.init.normal_(self.criteria_attention, 0, 0.01)
        nn.init.normal_(self.width_attention, 0, 0.01)
        self.apply(initialize_resnet)   

    def arch_parameters(self):
        return [self.width_attention] + [self.criteria_attention]
    ...
    def get_flop(self, mode, config_dict, extra_info):
        ...
        channels = [3]
        probe_list = []
        for i, weight in enumerate(self.width_attention):
            if mode == 'genotype':
                with torch.no_grad():
            ...
            probe_list.append(d)
        criterions = []
        for i, weight in enumerate(self.criteria_attention):
            with torch.no_grad():
                probe = nn.functional.softmax(weight, dim = 0)
    ...
    def search_forward(self, inputs):
        ...
        criteria_probs = nn.functional.softmax(self.criteria_attention, dim = 1)
        selected_widths, selected_probs = selectwithP(self.width_attention, self.tau, num = 2)
        ...
        for i, layer in enumerate(self.layers):
            ...
            probability = ratio_probs[last_channel_idx:last_channel_idx+layer.num_conv]
