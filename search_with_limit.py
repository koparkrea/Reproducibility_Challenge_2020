...
def pick_one_ratio(width_attention):
    width_attention_np = width_attention.cpu().detach().numpy()
    index_list = []
    ratios_ = [0.3, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 1.0]
    for layer in width_attention_np:
        index = np.argmax(layer)
        picked = ratios_[index]
        index_list.append(picked)
    return index_list
...
def main(args):
    ...
    model_base_path, model_best_path = os.path.join(os.getcwd(),'result','state_dict','search_limit','checkpoint.th'), os.path.join(os.getcwd(),'result','state_dict','search_limit', 'best.th')
    ...
    if args.pretrain:
        pretrain_path = os.path.join(os.getcwd(),'result','state_dict','basic_limit','best.th')
    ...
    # load checkpoint
    if args.resume:
        resume_path = os.path.join(os.getcwd(), 'result','state_dict', 'search_limit', 'checkpoint.th')
